...
for cell_i = 1:size(cell_to_index_map)
    if cell_to_index_map(cell_i,3)>0 && cell_to_index_map(cell_i,4)>0 && cell_to_index_map(cell_i,5)>0 && cell_to_index_map(cell_i,6)>0 % Select only cells active from day 5 to 10
        cellIndex_3456(end+1,:) = cell_i;
    end
end
...
%% Day 3-5
ct=1;
for cell_i = 1:length(cellIndex_3456)
    [~, ~, ~, ~, TF_3] = extract_1D_information(binarizedData{3}(:,cell_to_index_map(cellIndex_3456(cell_i),3)), cum_time{3}, cum_temporal_bin_vector, running_ts{3});
    TF_3 = smoothPlaceField(TF_3);
    TF_3 = TF_3-min(TF_3);
    TF_3 = TF_3./max(TF_3);
    
    [~, ~, ~, ~, TF_4] = extract_1D_information(binarizedData{4}(:,cell_to_index_map(cellIndex_3456(cell_i),5)), cum_time{4}, cum_temporal_bin_vector, running_ts{4});
    TF_4 = smoothPlaceField(TF_4);
    TF_4 = TF_4-min(TF_4);
    TF_4 = TF_4./max(TF_4);
    
    [~, ~, ~, ~, TF_5] = extract_1D_information(binarizedData{5}(:,cell_to_index_map(cellIndex_3456(cell_i),5)), cum_time{5}, cum_temporal_bin_vector, running_ts{5});
    TF_5 = smoothPlaceField(TF_5);
    TF_5 = TF_5-min(TF_5);
    TF_5 = TF_5./max(TF_5);
    
    [~, ~, ~, ~, TF_6] = extract_1D_information(binarizedData{6}(:,cell_to_index_map(cellIndex_3456(cell_i),6)), cum_time{6}, cum_temporal_bin_vector, running_ts{6});
    TF_6 = smoothPlaceField(TF_6);
    TF_6 = TF_6-min(TF_6);
    TF_6 = TF_6./max(TF_6);

    stability_35(ct) = corr(TF_5,TF_3);
    stability_46(ct) = corr(TF_6,TF_4);
    ct = ct+1;
end

save([workingFolder filesep 'LMT_temporal_stability_3456.mat'],'stability_35','stability_46', 'mouse_number')
