...
placeCellsProper_1 = spatial_pvalues{1} < 0.05;
placeCellsProper_3 = spatial_pvalues{3} < 0.05;
placeCellsProper_4 = spatial_pvalues{4} < 0.05;
...
for cell_i = 1:size(cell_to_index_map)
    if cell_to_index_map(cell_i,1)>0 && cell_to_index_map(cell_i,2)>0 % Select only cells active from day 3 (place cells) to 5
        if placeCellsProper_1(cell_to_index_map(cell_i,1)) == 1
    ...
    if cell_to_index_map(cell_i,3)>0 && cell_to_index_map(cell_i,5)>0 % Select only cells active from day 3 (place cells) to 5
        if placeCellsProper_3(cell_to_index_map(cell_i,3)) == 1
    ...
    if cell_to_index_map(cell_i,4)>0 && cell_to_index_map(cell_i,6)>0 % Select only cells active from day 4 (place cells) to 6
        if placeCellsProper_4(cell_to_index_map(cell_i,4)) == 1
...
for cell_i = 1:length(cellIndex_12)
    [~, ~, ~, ~, PF_1] = extract_1D_information(binarizedData{1}(:,cell_to_index_map(cellIndex_12(cell_i),1)), interp_behav_vec{1}, bin_vector, running_ts{1});
    PF_1 = smoothPlaceFieldParam(PF_1,5);
    PF_1 = PF_1-min(PF_1);
    PF_1 = PF_1./max(PF_1);
    placeFields1(:,end+1) = PF_1;
    
    [~, ~, ~, ~, PF_2] = extract_1D_information(binarizedData{2}(:,cell_to_index_map(cellIndex_12(cell_i),2)), interp_behav_vec{2}, bin_vector, running_ts{2});
    PF_2 = smoothPlaceFieldParam(PF_2,5);
    PF_2 = PF_2-min(PF_2);
    PF_2 = PF_2./max(PF_2);
    placeFields2(:,end+1) = PF_2;

    stability_12(cell_i) = corr(PF_2,PF_1);
    ct = ct+1;   
end
...
for cell_i = 1:length(cellIndex_35)
    ...
    PF_3 = smoothPlaceFieldParam(PF_3,5);
    PF_3 = PF_3-min(PF_3);
    PF_3 = PF_3./max(PF_3);
    ...
    PF_5 = smoothPlaceFieldParam(PF_5,5);
    PF_5 = PF_5-min(PF_5);
    PF_5 = PF_5./max(PF_5);
...
for cell_i = 1:length(cellIndex_46)
    ...
    PF_4 = smoothPlaceFieldParam(PF_4,5);
    PF_4 = PF_4-min(PF_4);
    PF_4 = PF_4./max(PF_4);
    ...
    PF_6 = smoothPlaceFieldParam(PF_6,5);
    PF_6 = PF_6-min(PF_6);
    PF_6 = PF_6./max(PF_6);
...
save([workingFolder filesep 'R1_LMT_spatial_stability_12_35_46_CPCs.mat'],'stability_12', 'stability_35','stability_46', 'placeFields1','placeFields2','placeFields3', 'placeFields4', 'placeFields5', 'placeFields6', 'mouse_number')

%% Plot here
figure
%% 1-2
[~, maxIdx] = max(placeFields1);
[~, sortedIdx] = sort(maxIdx,'descend');

subplot(2,2,1) 
imagesc(placeFields1(:,sortedIdx)')
colormap Viridis

subplot(2,2,2) 
imagesc(placeFields2(:,sortedIdx)')
colormap Viridis
