...
for currentDay = 1:10
    ...
    for cell_i = 1:numNeurons{currentDay}
        cumSig = cumsum(binarizedData{currentDay}(:,cell_i) & running_ts{currentDay});
        halfFrame = dsearchn(cumSig,cumSig(end)/2);
        sample_A = running_ts{currentDay};
        sample_B = sample_A;
        sample_A(halfFrame+1:end) = 0;
        sample_B(1:halfFrame) = 0;
        
        [~, ~, ~, ~, PF] = extract_1D_information(binarizedData{currentDay}(:,cell_i), interp_behav_vec{currentDay}, bin_vector, sample_A);
        vec_A = smoothPlaceField(PF);
        [~, ~, ~, ~, PF] = extract_1D_information(binarizedData{currentDay}(:,cell_i), interp_behav_vec{currentDay}, bin_vector, sample_B);
        vec_B = smoothPlaceField(PF);
        eachDay_LT_SplitHalfStab{currentDay}(cell_i) = corr(vec_A,vec_B);
    end
end

save([workingFolder filesep 'eachDay_LT_SplitHalfStab.mat'],'eachDay_LT_SplitHalfStab', 'eachDayLabels', 'mouse_number')