...
for cell_i = 1:size(cell_to_index_map)
    if sum(cell_to_index_map(cell_i,5:6)==0) == 0 % Select only cells active from day 5 to 10
...
for cell_i = 1:length(cellIndex)
    ...
    [~, ~, ~, ~, PF_6] = extract_1D_information(binarizedData{6}(:,cell_to_index_map(cellIndex(cell_i),6)), interp_behav_vec{6}, bin_vector, running_ts{6});
    PF_6 = smoothPlaceField(PF_6);
    PF_6 = PF_6-min(PF_6);
    PF_6 = PF_6./max(PF_6);
    
    stability = corr(PF_5,PF_6);
...
save([workingFolder filesep 'remapping_days5to6Ref.mat'],'stablePF', 'mouse_number')
