...
for folder_i = 1:length(sorted_dates)
    ...
    if folder2process(currentFolder)
        ...
        load([workingFolder filesep session_folders(currentFolder).name '/LMT_results_10s.mat'])
...
% Cells have to be present in all sessions, and be a place cell in at least
% one baseline session
cellIndex = [];
for cell_i = 1:size(cell_to_index_map)
    if cell_to_index_map(cell_i,1)>0 && cell_to_index_map(cell_i,2)>0 && cell_to_index_map(cell_i,3)>0 && cell_to_index_map(cell_i,5)>0 && cell_to_index_map(cell_i,4)>0 && cell_to_index_map(cell_i,6)>0 % Select only cells active from day 5 to 10
        if timeCellsProper_1(cell_to_index_map(cell_i,1)) == 1 || timeCellsProper_3(cell_to_index_map(cell_i,3)) == 1 || timeCellsProper_4(cell_to_index_map(cell_i,4)) == 1
...
for cell_i = 1:length(cellIndex)
    [~, ~, ~, ~, TF_1] = extract_1D_information(binarizedData{1}(:,cell_to_index_map(cellIndex(cell_i),1)), cum_time{1}, cum_temporal_bin_vector, running_ts{1});
    TF_1 = smoothPlaceField(TF_1);
    TF_1 = TF_1-min(TF_1);
    TF_1 = TF_1./max(TF_1);
    timeFields1(:,end+1) = TF_1;
    
    [~, ~, ~, ~, TF_2] = extract_1D_information(binarizedData{2}(:,cell_to_index_map(cellIndex(cell_i),2)), cum_time{2}, cum_temporal_bin_vector, running_ts{2});
    ...
    [~, ~, ~, ~, TF_3] = extract_1D_information(binarizedData{3}(:,cell_to_index_map(cellIndex(cell_i),3)), cum_time{3}, cum_temporal_bin_vector, running_ts{3});
    ...
    [~, ~, ~, ~, TF_4] = extract_1D_information(binarizedData{4}(:,cell_to_index_map(cellIndex(cell_i),4)), cum_time{4}, cum_temporal_bin_vector, running_ts{4});
    TF_4 = smoothPlaceField(TF_4);
    TF_4 = TF_4-min(TF_4);
    TF_4 = TF_4./max(TF_4);
    timeFields4(:,end+1) = TF_4;
    
    [~, ~, ~, ~, TF_6] = extract_1D_information(binarizedData{6}(:,cell_to_index_map(cellIndex(cell_i),6)), cum_time{6}, cum_temporal_bin_vector, running_ts{6});
...
save([workingFolder filesep 'LMT_temporal_stability_123546_TCs_only_10s.mat'],'stability_12', 'stability_35','stability_46', 'timeFields1', 'timeFields2', 'timeFields3', 'timeFields4', 'timeFields5', 'timeFields6', 'mouse_number')
