function extract_OF_BL_stability
...
%% Simulate 5s stimulations
stimulatedEpochs = logical(zeros(1,numFrames));
ct=0;
switcher = logical(0);
for frame = 1:numFrames
   ...
   stimulatedEpochs(frame) = switcher;
end
...
placeFields_A = zeros(length(X_bin_vector)-1, length(Y_bin_vector)-1, numNeurons)*nan;
placeFields_B = zeros(length(X_bin_vector)-1, length(Y_bin_vector)-1, numNeurons)*nan;
activity_A = zeros(1,numNeurons)*nan;
activity_B = zeros(1,numNeurons)*nan;
stability = zeros(1,numNeurons)*nan;

%% Create tuning curves
for cell_i = 1:numNeurons
    [~, ~, ~, activity_A(cell_i), PF_A ] = extract_2D_information(binarized_data(:,cell_i), interp_behav_vec, X_bin_vector, Y_bin_vector, sample_BL);
    PF_A = smoothPlaceField(PF_A);
    PF_A = PF_A-min(PF_A(:));
    placeFields_A(:,:,cell_i) = PF_A./max(PF_A(:),[],'omitnan');

    [~, ~, ~, activity_B(cell_i), PF_B ] = extract_2D_information(binarized_data(:,cell_i), interp_behav_vec, X_bin_vector, Y_bin_vector, sample_STIM);
    PF_B = smoothPlaceField(PF_B);
    PF_B = PF_B-min(PF_B(:));
    placeFields_B(:,:,cell_i) = PF_B./max(PF_B(:),[],'omitnan');
    
    stability(cell_i) = corr(PF_A(:),PF_B(:));
end

save([workingDir filesep 'OF_stability_baseline.mat'],'stability', 'activity_A','activity_B', 'placeFields_A','placeFields_B')
