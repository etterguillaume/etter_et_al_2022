...
for currentDay = 1:10
    ...
    for cell_i = 1:length(cellIndex)
        [overDays_LT_MI(currentDay,cell_i), ~, ~, overDays_LT_prob(currentDay,cell_i), PF] = extract_1D_information(binarizedData{currentDay}(:,cell_to_index_map(cellIndex(cell_i),currentDay)), single_trajectory_vec{currentDay}, bin_vector, running_ts{currentDay});
        PF = smoothPlaceField(PF);
        overDays_LT_area(currentDay,cell_i) = sum(PF >= 0.5*max(PF))*binSize;     
    end
end

save([workingFolder filesep 'overDays_LT_properties.mat'],'overDays_LT_MI', 'overDays_LT_area', 'overDays_LT_prob', 'overDaysLabels', 'mouse_number')