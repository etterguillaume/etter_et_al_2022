...
eachDayLabels = {'Day 1','Day 2', 'Day 5', 'Day 6', 'Day 7, 8Hz stims', 'Day 8, scrambled stims'};

for day=1:6
        eachDay_LMT_RunActiveProb{day} = sum(binarizedData{day}(running_ts{day},:))/sum(running_ts{day});
        eachDay_LMT_IdleActiveProb{day} = sum(binarizedData{day}(~running_ts{day},:))/sum(~running_ts{day});
        eachDay_LMT_RunTransProb{day} = sum(diff(binarizedData{day}(running_ts{day},:))==1)/sum(running_ts{day});
        eachDay_LMT_IdleTransProb{day} = sum(diff(binarizedData{day}(~running_ts{day},:))==1)/sum(~running_ts{day});   
end
save([workingFolder filesep 'eachDay_LMT_activity.mat'],'eachDay_LMT_RunActiveProb', 'eachDay_LMT_IdleActiveProb', 'eachDay_LMT_RunTransProb', 'eachDay_LMT_IdleTransProb','eachDayLabels', 'mouse_number')