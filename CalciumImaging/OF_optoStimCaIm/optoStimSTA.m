function [STA_onset, STA_offset] = optoStimSTA(ms, behav, cell_ID, OpticSigThreshold)
...
binarizedTrace = extract_binary(calciumTrace,Fs,2);

numFrames = length(binarizedTrace);
...
for stim_i = 1:length(stimOnset_idx)
    if stimOnset_idx(stim_i) - StimDurationFrames <= 0 || stimOnset_idx(stim_i) + StimDurationFrames > length(stimOnset)
        ...
        STA_onset(stim_i,:) = binarizedTrace(stimOnset_idx(stim_i)-StimDurationFrames:stimOnset_idx(stim_i)+StimDurationFrames);
    end
end
...
for stim_i = 1:length(stimOffset_idx)
    if stimOffset_idx(stim_i) - StimDurationFrames <= 0 || stimOffset_idx(stim_i) + StimDurationFrames > length(stimOffset)
        ...
        STA_offset(stim_i,:) = binarizedTrace(stimOffset_idx(stim_i)-StimDurationFrames:stimOffset_idx(stim_i)+StimDurationFrames);
    end
end
