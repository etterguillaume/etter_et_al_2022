...
for cell_i = 1:length(cellIndex_3456)
    [~, ~, ~, ~, PF_3] = extract_1D_information(binarizedData{3}(:,cell_to_index_map(cellIndex_3456(cell_i),3)), interp_behav_vec{3}, bin_vector, running_ts{3});
    PF_3 = smoothPlaceField(PF_3);
    PF_3 = PF_3-min(PF_3);
    PF_3 = PF_3./max(PF_3);
    
    [~, ~, ~, ~, PF_4] = extract_1D_information(binarizedData{4}(:,cell_to_index_map(cellIndex_3456(cell_i),4)), interp_behav_vec{4}, bin_vector, running_ts{4});
    PF_4 = smoothPlaceField(PF_4);
    PF_4 = PF_4-min(PF_4);
    PF_4 = PF_4./max(PF_4);
    
    [~, ~, ~, ~, PF_5] = extract_1D_information(binarizedData{5}(:,cell_to_index_map(cellIndex_3456(cell_i),5)), interp_behav_vec{5}, bin_vector, running_ts{5});
    PF_5 = smoothPlaceField(PF_5);
    PF_5 = PF_5-min(PF_5);
    PF_5 = PF_5./max(PF_5);
    
    [~, ~, ~, ~, PF_6] = extract_1D_information(binarizedData{6}(:,cell_to_index_map(cellIndex_3456(cell_i),6)), interp_behav_vec{6}, bin_vector, running_ts{6});
    PF_6 = smoothPlaceField(PF_5);
    PF_6 = PF_6-min(PF_6);
    PF_6 = PF_6./max(PF_6);
...
save([workingFolder filesep 'LMT_spatial_stability_3456.mat'],'stability_35','stability_46', 'mouse_number')
