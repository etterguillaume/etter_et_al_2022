...
for cell_i = 1:size(binarized_data,2)
    [~, ~, occupancy_vector, prob_being_active(cell_i), tuning_curve_data(:,cell_i) ] = extract_1D_information(binarized_data(:,cell_i), cum_distance, cum_spatial_bin_vector, running_ts);
end
...
decoded_distance(~running_ts) = nan;
%decoded_probabilities(:,~running_ts) = nan;

%% Plot actual vs decoded time
figure
subplot(2,1,1)
imagesc(ca_time,cum_spatial_bin_centers_vector,decoded_probabilities)
colormap Viridis
...
ax1.YDir = 'normal';
subplot(2,1,2)
plot(ca_time,actual_distance)
hold on
plot(ca_time, decoded_distance)
title 'Actual versus decoded distance'
xlabel 'Time (s)'
ylabel 'Distance traveled (cm)'
ax2 = gca;
