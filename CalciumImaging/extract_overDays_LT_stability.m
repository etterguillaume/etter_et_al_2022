...
for currentDay = 1:10
    ...
    [~, ~, ~, ~, PF] = extract_1D_information(binarizedData{currentDay}(:,cell_to_index_map(cellIndex(cell_i),currentDay)), interp_behav_vec{currentDay}, bin_vector, sample_A);
    vec_A = smoothPlaceField(PF);
    [~, ~, ~, ~, PF] = extract_1D_information(binarizedData{currentDay}(:,cell_to_index_map(cellIndex(cell_i),currentDay)), interp_behav_vec{currentDay}, bin_vector, sample_B);
    vec_B = smoothPlaceField(PF);
    overDays_LT_stability(cell_i,currentDay) = corr(vec_A,vec_B);
    end
end

save([workingFolder filesep 'overDays_LT_stability.mat'],'overDays_LT_stability', 'overDaysLabels', 'mouse_number')