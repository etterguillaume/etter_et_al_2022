...
for currentDay = 1:10
    ...
    for cell_i = 1:length(cellIndex)
        ...
        [~, ~, ~, ~, PF] = extract_1D_information(binarizedData{currentDay}(:,cell_to_index_map(cellIndex(cell_i),currentDay)), single_trajectory_vec{currentDay}, bin_vector, running_ts{currentDay});
...
[~,idx] = max(overDays_LT_MinMaxFields{1});
[~, sortedIdx] = sort(idx,'descend');

save([workingFolder filesep 'overDays_LT_MinMaxFields.mat'],'overDays_LT_MinMaxFields', 'overDaysLabels', 'mouse_number')