...
decoding_ts(running_ts == 0) = 0; % Periods of immobility are excluded
    ...
    %% Decode  
    [spatial_actual{bootstrap_i}, spatial_decoded{bootstrap_i}, spatial_decoding_error{bootstrap_i}, spatial_decoding_agreement{bootstrap_i}] = decode_1D(binarized_data(:,cell_used), interp_behav_vec, spatial_bin_vector, cell_used, training_ts, decoding_ts);
    [spatial_state_actual{bootstrap_i}, spatial_state_decoded{bootstrap_i}, spatial_state_decoding_error{bootstrap_i}, spatial_state_decoding_agreement{bootstrap_i}] = decode_1D(binarized_data(:,cell_used), single_trajectory_vec, cum_spatial_bin_vector, cell_used, training_ts, decoding_ts);
    [distance_actual{bootstrap_i}, distance_decoded{bootstrap_i}, distance_decoding_error{bootstrap_i}, distance_decoding_agreement{bootstrap_i}] = decode_1D(binarized_data(:,cell_used), cum_distance, cum_spatial_bin_vector, cell_used, training_ts, decoding_ts);
    [time_actual{bootstrap_i}, time_decoded{bootstrap_i}, time_decoding_error{bootstrap_i}, time_decoding_agreement{bootstrap_i}] = decode_1D(binarized_data(:,cell_used), cum_time, cum_temporal_bin_vector, cell_used, training_ts, decoding_ts);
    disp(['Done: ' num2str(bootstrap_i) '/' num2str(numBootstraps)])
end

%% Confusion matrices here
spatial_confusion_matrix = zeros(length(spatial_bin_centers_vector),length(spatial_bin_centers_vector));

pooled_actual_spatial=[];
pooled_decoded_spatial=[];
for i = 1:numBootstraps
   pooled_actual_spatial = [pooled_actual_spatial spatial_actual{i}];
   pooled_decoded_spatial = [pooled_decoded_spatial spatial_decoded{i}];   
end

for actual_i = 1:length(spatial_bin_centers_vector)
   for decoded_i = 1:length(spatial_bin_centers_vector)
       spatial_confusion_matrix(actual_i,decoded_i) = sum(pooled_decoded_spatial == decoded_i & pooled_actual_spatial == actual_i)./sum(pooled_actual_spatial == actual_i);
   end
end

%% Time
temporal_confusion_matrix = zeros(length(cum_temporal_bin_centers_vector),length(cum_temporal_bin_centers_vector));

pooled_actual_temporal=[];
pooled_decoded_temporal=[];
for i = 1:numBootstraps
   pooled_actual_temporal = [pooled_actual_temporal; time_actual{i}];
   pooled_decoded_temporal = [pooled_decoded_temporal time_decoded{i}];   
end

pooled_actual_temporal=pooled_actual_temporal';

for actual_i = 1:length(cum_temporal_bin_centers_vector)
   for decoded_i = 1:length(cum_temporal_bin_centers_vector)
       temporal_confusion_matrix(actual_i,decoded_i) = sum(pooled_decoded_temporal == decoded_i & pooled_actual_temporal == actual_i)./sum(pooled_actual_temporal == actual_i);
   end
end
