...
for shuffle_i = 1:numBootstrapSamples
    ...
    for day = 5:10
        ...
        decoding_agreement{day-4}(shuffle_i) = sum(decoding_agreement_vector)./length(decoding_agreement_vector);
        
        %% Compute decoding error
        decoding_error = actual_position - decoded_position;
        mean_decoding_error{day-4}(shuffle_i) = mean(abs(decoding_error), 'omitnan');
    end
end

for day = 1:6
meanDailyError(day)=mean(mean_decoding_error{day});
end
figure
plot(meanDailyError);
