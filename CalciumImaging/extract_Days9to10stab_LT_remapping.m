...
for cell_i = 1:size(cell_to_index_map)
    if sum(cell_to_index_map(cell_i,9:10)==0) == 0 % Select only cells active from day 5 to 10
...
for cell_i = 1:length(cellIndex)
    [~, ~, ~, ~, PF_9] = extract_1D_information(binarizedData{9}(:,cell_to_index_map(cellIndex(cell_i),9)), interp_behav_vec{9}, bin_vector, running_ts{9});
    PF_9 = smoothPlaceField(PF_9);
    PF_9 = PF_9-min(PF_9);
    PF_9 = PF_9./max(PF_9);
...
save([workingFolder filesep 'remapping_days1to2Ref.mat'],'stablePF', 'mouse_number')

%% Plot here
figure
[~, maxIdx] = max(stablePF{10});
