...
for cell_i = 1:size(binarized_data,2)
    [~, ~, occupancy_vector, prob_being_active(cell_i), tuning_curve_data(:,cell_i) ] = extract_1D_information(binarized_data(:,cell_i), cum_time, cum_temporal_bin_vector, training_ts);
    tuning_curve_data(:,cell_i) = smoothPlaceFieldParam(tuning_curve_data(:,cell_i),10);
end
...
confusion_matrix = zeros(length(cum_temporal_bin_centers_vector),length(cum_temporal_bin_centers_vector));
...
figure
imagesc(cum_temporal_bin_centers_vector, cum_temporal_bin_centers_vector, confusion_matrix)
colormap viridis
