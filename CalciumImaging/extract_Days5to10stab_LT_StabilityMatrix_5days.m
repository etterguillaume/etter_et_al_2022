...
for day_i = 5:10
    for day_j = 5:10
        ...
        for cell_i = 1:length(cellIndex)
            if cell_to_index_map(cellIndex(cell_i),day_i)>0 && cell_to_index_map(cellIndex(cell_i),day_j)>0
               ...
               stabilityMatrix{day_i-4,day_j-4}(ct) = corr(PF_A,PF_B);
               ct=ct+1;
            else
               stabilityMatrix{day_i-4,day_j-4}(ct) = NaN;
               ct=ct+1;
            end
        end
        meanStabilityMatrix(day_i-4,day_j-4) = mean(stabilityMatrix{day_i-4,day_j-4},'omitnan');
    end
end
    
    save([workingFolder filesep 'stabilityMatrix_Day5to10.mat'],'stabilityMatrix', 'meanStabilityMatrix', 'mouse_number')
    
    figure
    imagesc(meanStabilityMatrix)
    colormap Viridis
    daspect([1 1 1])